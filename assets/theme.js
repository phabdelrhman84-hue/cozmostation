...
(function($){
  ...
  theme.FreeShippingBar = (function() {
    var config = {
      threshold: 250000, // 2500 EGP in cents (Shopify stores prices in cents)
      currency: 'EGP',
      selectors: {
        bar: '#FreeShippingBar',
        message: '#FreeShippingMessage',
        fill: '#FreeShippingProgress',
        track: '.free-shipping-bar__track'
      }
    };

    function formatAmount(cents) {
      // Format as EGP with comma separators - no decimal for clean look
      var amount = Math.ceil(cents / 100);
      return amount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',') + ' ' + config.currency;
    }

    function update(cart) {
      var $bar = $(config.selectors.bar);
      var $message = $(config.selectors.message);
      var $fill = $(config.selectors.fill);
      var $track = $(config.selectors.track);

      if (!$bar.length) return;

      var totalPrice = cart.total_price || 0;
      ...
      var remaining = config.threshold - totalPrice;
      var percentage = Math.min((totalPrice / config.threshold) * 100, 100);

      // Update ARIA
      $track.attr('aria-valuenow', Math.round(percentage));

      if (remaining <= 0) {
        // Threshold met — show success state
        $message.html('\u0645\u0628\u0631\u0648\u0643! \u0644\u0642\u062F \u062D\u0635\u0644\u062A \u0639\u0644\u0649 <strong>\u0634\u062D\u0646 \u0645\u062C\u0627\u0646\u064A</strong> \uD83D\uDE9A');
        $fill.css('width', '100%');
        $bar.addClass('free-shipping-bar--achieved').removeClass('free-shipping-bar--in-progress');
      } else {
        // Still progressing
        var remainingFormatted = formatAmount(remaining);
        $message.html('\u0623\u0636\u0641 <strong>' + remainingFormatted + '</strong> \u0644\u0644\u062D\u0635\u0648\u0644 \u0639\u0644\u0649 <strong>\u0634\u062D\u0646 \u0645\u062C\u0627\u0646\u064A</strong>');
        $fill.css('width', percentage + '%');
        $bar.addClass('free-shipping-bar--in-progress').removeClass('free-shipping-bar--achieved');
      }
    }
  ...
  // =============================================
  // Cart Upsell Recommendations Module
  // =============================================
  theme.CartUpsells = (function() {
    var selectors = {
      container: '#CartUpsells',
      items: '#CartUpsellItems',
      scrollable: '#CartContainer .drawer__scrollable',
      footer: '#CartContainer .drawer__footer'
    };

    var maxItems = 3;

    function relocate() {
      // Move the upsell container into the scrollable area, right before the footer
      var $container = $(selectors.container);
      var $scrollable = $(selectors.scrollable);
      var $footer = $(selectors.footer);

      if (!$container.length || !$scrollable.length) return;

      // Append upsells at the end of the scrollable area (before footer, which is outside scrollable)
      // Actually footer is a sibling of scrollable inside drawer__inner,
      // so we append to scrollable to keep it in the scroll flow
      $scrollable.append($container);
    }

    function buildItemHtml(product) {
      var img = product.featured_image || '';
      if (img) {
        img = img.replace(/(\.[^.]*)$/, '_120x$1');
      } else {
        img = '//cdn.shopify.com/s/assets/admin/no-image-medium-cc9732cb976dd349a0df1d39816fbcc7.gif';
      }

      var variantId = product.variants[0].id;
      var price = theme.Currency.formatMoney(product.variants[0].price, theme.settings.moneyFormat);

      var html = '';
      html += '<div class="cart-upsell-item" data-variant-id="' + variantId + '">';
      html += '  <a href="' + product.url + '" class="cart-upsell-item__image-link">';
      html += '    <img src="' + img + '" alt="' + product.title + '" class="cart-upsell-item__image" loading="lazy" />';
      html += '  </a>';
      html += '  <div class="cart-upsell-item__info">';
      html += '    <a href="' + product.url + '" class="cart-upsell-item__title">' + product.title + '</a>';
      html += '    <span class="cart-upsell-item__price">' + price + '</span>';
      html += '  </div>';
      html += '  <button type="button" class="cart-upsell-item__add js-upsell-add" data-variant-id="' + variantId + '" aria-label="\u0623\u0636\u0641">';
      html += '    <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M7 1v12M1 7h12" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>';
      html += '  </button>';
      html += '</div>';
    ...
    function update(cart) {
      ...
      // Relocate into the scrollable area each time cart rebuilds
      relocate();

      // Collect product IDs currently in cart to filter them out
      var cartProductIds = [];
      for (var i = 0; i < cart.items.length; i++) {
        cartProductIds.push(cart.items[i].product_id);
      }

      // Use the first cart item's product ID to get recommendations
      var sourceProductId = cart.items[0].product_id;

      // Fetch recommendations from Shopify Product Recommendations API
      $.getJSON('/recommendations/products.json?product_id=' + sourceProductId + '&limit=10')
        .then(function(response) {
          ...
          for (var j = 0; j < products.length; j++) {
            if (cartProductIds.indexOf(products[j].id) === -1 && products[j].available) {
          ...
          // Bind add buttons
          $items.find('.js-upsell-add').off('click.upsell').on('click.upsell', function(evt) {
            evt.preventDefault();
            var $btn = $(this);
  ...
  theme.CartDrawer = (function() {
    ...
    CartDrawer.prototype = $.extend({}, CartDrawer.prototype, {
      ...
      buildCart: function(cart, openDrawer) {
        ...
        theme.FreeShippingBar.update(cart);

        // === Cart Upsell Recommendations Update ===
        theme.CartUpsells.update(cart);
    ...
    Product.prototype = $.extend({}, Product.prototype, {
      ...
      updateInventory: function(evt) {
        ...
        if (variant.inventory_management === 'shopify' && window.inventories && window.inventories[this.sectionId]) {
          ...
          this.toggleInventoryQuantity(true, quantity);
  
          if (quantity <= 0 && variantInventoryObject.incoming) {
      ...
      toggleInventoryQuantity: function(show, qty) {
        if (!show || typeof qty === 'undefined' || qty <= 0) {
        ...
        if (qty > 0 && qty < 15) {
          $(this.selectors.inventory)
            .removeClass('hide stock-green')
            .addClass('stock-red stock-pulse')
            .html('🔥 يلا بسرعه ! باقي <strong>' + qty + '</strong> قطع فقط في المخزن.');
        } else if (qty >= 15) {
          $(this.selectors.inventory)
            .removeClass('hide stock-red stock-pulse')
            .addClass('stock-green')
            .html('✔️ متوفر في المخزن - جاهز للشحن.');
        } else {
          $(this.selectors.inventory).addClass('hide');
        }
      },
